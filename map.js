...
function displayPlaces(places) {
    ...
    removeMarker();
    kakaoUserLocation = new kakao.maps.LatLng(userLocation.latitude, userLocation.longitude);
    for (var i = 0; i < places.length; i++) {
        ...
        places[i].distance = Math.round(line.getLength());
...
function getListItem(index, places) {
    ...
    if (places.road_address_name) {
        itemStr += `<span>${places.road_address_name}</span><span class="jibun gray">${places.address_name}</span><br>`;
    } else {
    ...
    if (places.phone) {
        ...
        itemStr += `<span class="tel">* 전화번호를 찾을 수 없습니다.</span>&nbsp&nbsp&nbsp<span class="distance">${places.distance}m</span></div>`;
    }
